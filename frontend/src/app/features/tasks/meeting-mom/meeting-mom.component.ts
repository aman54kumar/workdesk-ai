...
@Component({
  ...
  template: `
    <div class="page-wrap">
      ...
      <p class="page-desc">Turn rough meeting notes into structured, shareable Minutes of Meeting</p>

      <div class="mb-4">
        <label class="field-label">Paste your rough meeting notes</label>
        <textarea
          [(ngModel)]="input"
          (ngModelChange)="onInputChange()"
          rows="9"
          placeholder="e.g. Date: 15 May 2026. Attendees: project lead, developer, client contact. Discussed milestone delay. Client needs UAT by 30 June. Agreed to share test credentials by 20 June."
          class="field-input"
        ></textarea>
      ...
      <app-streaming-output
        ...
        taskType="meeting_mom" [model]="lastModel()"
        (regenerate)="onGenerate()" (retry)="onGenerate()" (cancelQueue)="onCancelQueue()"
      />
    </div>
  `,
})
export default class MeetingMomComponent implements OnInit {
  ...
  onGenerate(): void {
    ...
    this.cancelGen = startToolGeneration(
      this.gs, this.history, 'meeting_mom', { input: this.input.trim() },
      {
